...
export function getAllBoxByIdInBD(idUser: string) {
  return new Promise((res, err) => {
    ...
    con.connect((e) => {
      ...
      var sql = `
      SELECT hb2.idBox,hb2.idUser,us.nameUser,us.avatar,hb1.status 
      FROM havelistboxchat hb1, havelistboxchat hb2, user us, boxchat b
      WHERE hb1.idUser= ? AND hb2.idUser <> ? AND hb1.idBox=hb2.idBox AND us.id=hb2.idUser AND hb1.status <> 0 AND 	  	hb1.idBox=b.idBox
...
export function GetEmptyBoxDB() {
  return new Promise((res, err) => {
    ...
    con.connect((e) => {
      ...
      var sql =
        "SELECT idBox FROM boxchat WHERE idBox NOT IN (SELECT idBox FROM havelistboxchat GROUP BY idBox) LIMIT 1";
      con.query(sql, (e, rt, fiels) => {
        if (e) {
          err(e.message);
        }
...
export function UpdateDateBoxDB(idBox: string) {
  return new Promise((res, err) => {
    var con = mysql.createConnection(confi);
    con.connect((e) => {
      if (e) {
