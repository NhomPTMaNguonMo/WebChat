...
export function InsertContentInDB(idBox: string, idUser: string, mess: string) {
  return new Promise((res, rej) => {
    ...
    con.connect((err) => {
      ...
      var sql =
        "INSERT INTO messenge ( idBox, content, type,idUser) VALUES (?,?,0,?)";
...
export function IsMessInBoxDB(idBox: string, idMess: string) {
  return new Promise((res, rej) => {
    var con = mysql.createConnection(confi);
    con.connect((err) => {
      if (err) {
...
export function DeleteMessbyIdUserDB(idUser: string, idMess: string){
    return new Promise((res, rej) => {
        var con = mysql.createConnection(confi);
        con.connect((err) => {
          if (err) {
